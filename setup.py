...
# Read the content of src/linkup/_version.py for the version, without importing the module
# This assumes the version is defined as `__version__ = "x.y.z"` (support type hint or inline
# comment at the end of the line)
version: Optional[str] = None
with open("src/linkup/_version.py", "r") as version_file:
    for line in version_file.readlines():
        if line.startswith("__version__"):
            version = line.split("=")[1].split("#")[0].strip().strip('"').strip("'")
            break
if version is None:
    raise ValueError("Could not determine the version from src/linkup/_version.py")
